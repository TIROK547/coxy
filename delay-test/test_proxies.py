...
# github repository: https://github.com/exelite-dev/Proxy-Checker/blob/main/telegram_proxy_checker.py
...
DEFAULT_INPUT = os.getenv("RAW_PROXIES_FILE", "output/raw_proxies.txt")
DEFAULT_OUTPUT = os.getenv("OUTPUT_PROXIES_FILE", "output/result_proxies.txt")
DEFAULT_TIMEOUT = 5.0
...
def save_working(results: list[CheckResult], output_path: Path) -> int:
    ...
    #     "",
    # ] uncomment if you need summery in result file

    for item in working:
        #lines.append(f"{item.proxy.display()} | {item.latency_ms:.1f} ms") uncomment if you need latency in result
        lines.append(f"{item.proxy.display()}")
